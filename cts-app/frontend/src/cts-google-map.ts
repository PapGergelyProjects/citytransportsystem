...
export class CtsGoogleMap extends LitElement{
    ...
    protected async initMap(data: InitMapData) {
        let loadGoogleApi = new Loader(this.apiKey, this.opts);
        this.googleLoader = await loadGoogleApi.load();
        console.log("googleLoader: ", this.googleLoader);
        let googleMapDiv: Element | null | undefined = this.shadowRoot?.querySelector("#ctsMap");
        let map = new this.googleLoader.maps.Map(googleMapDiv!, {
            center: {lat:data.coordinate.lat, lng:data.coordinate.lng},
            zoom: data.zoomMagnitude
        });
        this.googleLoader.maps.event.addListener(map, "click", (e) => {
    ...
    protected async addMarker(title: string, coordinate: Coordinate) {
        let marker = new this.googleLoader.maps.Marker({
    ...
    protected async drawCircle(center: Coordinate){
        let circle = new this.googleLoader.maps.Circle({
            map: this.googleMap,
            strokeColor: '#6600cc',
            fillOpacity: 0.1,
